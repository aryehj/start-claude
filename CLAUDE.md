...
- **Container runs with `--cap-add SYS_ADMIN` so the bubblewrap sandbox can create namespaces.** Apple Containers' default seccomp profile gates `clone(CLONE_NEW*)` on `CAP_SYS_ADMIN` (permitting only userns-alone without it), so `bwrap` failed its preflight without this. Safe here — unlike the shared-VM Colima case in ADR-033 — because each container is its own microVM, so the cap empowers the process only within its dedicated VM, not against the macOS host. Stock non-setuid `bwrap` is sufficient once the cap is present. See ADR-043.
